...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    if [ "$GRADLE_CYGWIN_VERBOSE" = "true" ] ; then
        echo "using OURCYGPATTERN=$OURCYGPATTERN"
    fi
    CLASSPATH=`cygpath --path --mixed $CLASSPATH`
    if [ -n "$CLASSPATH" ] ; then
        CLASSPATH=`cygpath --path --mixed "$CLASSPATH"`
...
# traverse instead of using $@ to lose quoting in sh褔
JAVA_ARGS=()
for arg do
    if [[ $arg =~ ^-([Dd]([jJ][vV][mM]|[eE]|[pP])|[oO]|--) ]] || [[ $arg == @* ]]; then
        JAVA_ARGS+=("$arg")
    else
        break
    fi
...
# if $@, then we have explicit args and we can bypass the trap
set -- "${JAVA_ARGS[@]}" -classpath "$CLASSPATH" org.gradle.wrapper.GradleWrapperMain "$@"
